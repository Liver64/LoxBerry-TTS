...
if (!$reuse_client) {
  ...
  sh("chmod 0640 ".esc($CLI_KEY));
  my ($rcsr,$oc) = sh("openssl req -new -key ".esc($CLI_KEY)." -subj ".esc("/CN=sip_bridge")." -out ".esc($CLI_CSR));
...
if ($bundle) {
  my $bundle_path = File::Spec->catfile($BRIDGE_DIR, "sip_bundle.tar.gz");
  my $tmpdir = "/tmp/sip_bundle.$$";
  ensure_dir($tmpdir);
  my $cli_rel = "clients/sip_bridge";
  ...
  if (!$rt) {
    logp("OK","Bundle created: $bundle_path (contains mosq-ca.crt, t2s.crt, t2s.key, master.info, clients/sip_bridge/*)");
  } else {
