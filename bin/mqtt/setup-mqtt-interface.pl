...
my $acl = <<"ACL";
# Auto-generated by Text2Speech Plugin (stable & permissive)

# =====================================================================
# /etc/mosquitto/tts-aclfile
# -- Base ACL (DO NOT EDIT MANUALLY) ----------------------------------
# This block is static and created during setup. Dynamic entries will be
# appended inside the AUTO-ACL section at runtime.
# =====================================================================

### BEGIN BASE-ACL ###

# --- Bridge Clients (base CN) ---
user $CLIENT_ID
...
# --- Allow all authenticated mTLS clients to perform handshake ---
pattern write tts-handshake/#
pattern read  tts-handshake/#
...
# --- Ensure ACL file is writable by the handshake listener ---
system("chown loxberry:loxberry " . esc($acl_file_path));
system("chmod 664 " . esc($acl_file_path));
logp("OK","ACL permissions set for listener (loxberry:loxberry, 664)");
...
if ($bundle) {
  ...
  # Prüfen, ob Bundle bereits existiert und Zertifikate neu erstellt wurden
  if (!$reuse_server || !$reuse_client || !$have_ca) {
      $needs_bundle = 1;
      logp("INFO","Certificates changed – bundle will be rebuilt.");
  } else {
      # Bundle fehlt oder ist leer → ebenfalls neu bauen
      if (!-f $bundle_path || -z $bundle_path) {
          $needs_bundle = 1;
          logp("INFO","Bundle missing or empty – will be rebuilt.");
      } else {
          logp("INFO","No certificate changes detected – existing bundle kept: $bundle_path");
      }
  }
  ...
  logp("INFO","--no-bundle: skipping bundle generation");
}
